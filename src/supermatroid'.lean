...
def supermatroid.has_maximal_le {α : Type u} [preorder α] (s : set α) : Prop := 
  ∀ (a : α), (maximals (≤) (s ∩ set.Iic a)).nonempty  

def supermatroid.has_maximal_Icc_of_Iic {α : Type u} [preorder α] (s : set α) : Prop := 
  ∀ (a x : α), a ≤ x → (s ∩ maximals (≤) (set.Iic x)).nonempty 
     → (s ∩ maximals (≤) (set.Icc a x)).nonempty 
...
(ind_has_basis_of   : supermatroid.has_maximal_le ind)
(ind_has_extension  : supermatroid.has_maximal_Icc_of_Iic ind)
...
def spanning_sets (M : supermatroid α) : set α := M.spanning  

def coindep [has_dual_iso α] (M : supermatroid α) (x : α) := 
  ∃ b, x ≤ φ b ∧ M.basis b 

def coindep_sets [has_dual_iso α] (M : supermatroid α) : set α := M.coindep 

def cobases [has_dual_iso α] (M : supermatroid α) := maximals (≤) M.coindep_sets 

def cobasis [has_dual_iso α] (M : supermatroid α) : α → Prop := M.cobases 

@[simp] lemma mem_ind_iff : i ∈ M.ind ↔ M.indep i := iff.rfl   

@[simp] lemma mem_bases_iff : b ∈ M.bases ↔ M.basis b := iff.rfl 

def cospanning [has_dual_iso α] (M : supermatroid α) : α → Prop := λ x, ∃ b, b ≤ x ∧ M.cobasis b 

def cospanning_sets [has_dual_iso α] (M : supermatroid α) : set α := M.cospanning
...
begin
  obtain ⟨a,⟨⟨ha1,ha2⟩,-⟩⟩ := M.ind_has_basis_of ⊥, 
  exact (mem_singleton_iff.mp (Iic_bot.subst ha2)).subst ha1, 
end 
...
lemma indep.basis_of (hi : M.indep i) (hix : i ≤ x) (h : ∀ j, M.indep j → i ≤ j → j ≤ x → i = j) : 
  M.basis_of i x :=
⟨⟨hi,hix⟩, λ j h' h'', h j h'.1 h'' h'.2⟩

lemma indep.basis (hi : M.indep i) (hmax : ∀ j, M.indep j → i ≤ j → i = j) : M.basis i := 
⟨hi, λ j, hmax j⟩

lemma coind_blah [has_dual_iso α] (M : supermatroid α) (hix : i ≤ x) (hi : M.coindep i) 
  (hx : M.cospanning x): 
  (M.cobases ∩ (Icc i x)).nonempty := 
begin
  --intros i hi j hic_nb hjc_b, 
  obtain ⟨b,hb_b,hbi⟩ := hi, 
  --obtain ⟨b',hb'x, ⟨h⟩ := hx, 
