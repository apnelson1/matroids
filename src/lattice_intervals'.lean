...
instance boolean_algebra' [boolean_algebra α] : boolean_algebra (Icc a (a ⊔ b)) := 
{ compl := λ x, ⟨a ⊔ (xᶜ ⊓ b), ⟨le_sup_left,by {apply sup_le_sup_left _, }⟩⟩,
  sdiff := λ x y, x ⊓ ⟨a ⊔ (yᶜ ⊓ b), ⟨le_sup_left, sorry⟩⟩,
...
--   left_inv := by {rintro ⟨x,hx⟩, simp only [subtype.coe_mk, subtype.mk_eq_mk, h₀ x a b hx.1 hx.2],},
--   right_inv := by {rintro ⟨x,hx⟩, simp only [subtype.coe_mk, subtype.mk_eq_mk], 
--     convert h₀ x _ _ hx.1 hx.2; simp},
--   map_rel_iff' := 
--   begin 
