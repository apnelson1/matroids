...
class basis_family (α : Type u) [has_le α] := 
(carrier : set α)
(nonempty : carrier.nonempty)
(is_antichain : is_antichain (≤) carrier)

variables [partial_order α] [basis_family α] {x y i j a b b' c s t f d : α}
...
def basis_of (i x : α) := indep i ∧ i ≤ x ∧ ∀ j, indep j → j ≤ x → i ≤ j → i = j 
...
@[simp] lemma mem_lower_set_basis_iff_indep : indep i ↔ i ∈ lower_set_of (basis : set α) := 
⟨λ ⟨b,hb,hbx⟩, ⟨b,hb,hbx⟩, λ ⟨b,hb,hbx⟩, ⟨b,hb,hbx⟩⟩

lemma exists_basis : ∃ (b : α), basis b := basis_family.nonempty
...
lemma bot_indep [order_bot α]: indep (⊥ : α) := 
exists.elim exists_basis (λ b h, ⟨b, h, bot_le⟩)
...
lemma indep.basis_of (hi : indep i) (hix : i ≤ x) (h : ∀ j, indep j → j ≤ x → i ≤ j → i = j) : 
  basis_of i x :=
⟨hi,  hix, h⟩

lemma basis.indep (h : basis b) : indep b := ⟨b, h, rfl.le⟩ 

lemma exists_indep : ∃ (i : α), indep i := exists_basis.imp (λ b hb, ⟨b,hb,rfl.le⟩)

lemma basis.indep_of_le (hb : basis b) (hi : i ≤ b) : indep i := ⟨b,hb,hi⟩ 

lemma basis.eq_of_le_indep (h : basis b) (hbi : b ≤ i) (hi : indep i) : b = i := 
begin
  by_contradiction h_ne, 
  obtain ⟨b',h₁,h₂⟩ := hi, 
  exact basis_antichain h h₁ (λ hbb', h_ne (hbi.antisymm (hbb'.substr h₂))) (hbi.trans h₂),
end 

lemma basis.not_indep_of_lt (hb : basis b) (hbx : b < x) : ¬ indep x := 
λ hx, hbx.ne (by {have := hb.eq_of_le_indep, })

lemma basis.eq_of_basis_le (hb : basis b) (hx : basis x) (hxb : x ≤ b) : x = b :=
...
lemma basis_of.indep (h : basis_of b x) : indep b := h.1

lemma basis_of.le (h : basis_of b x) : b ≤ x := h.2.1

lemma basis_of.not_indep_of_lt (h : basis_of b x) (hby : b < y) (hyx : y ≤ x) : ¬ indep y := 
begin
  rintros ⟨b',hb',hyb'⟩, 
  obtain ⟨i,hix,hi⟩ := h, 
...
lemma basis_of.eq_of_le_indep (h : basis_of b x) (hby : b ≤ y) (hYx : y ≤ x) (hy : indep y): 
  b = y := 
by_contra (λ h', h.not_indep_of_lt (lt_of_le_of_ne hby h') hYx hy)

lemma basis_of.indep_of_le (hb : basis_of b x) (hib : i ≤ b)  : indep i := 
(hb.indep).indep_of_le hib 

lemma basis.basis_of_top [order_top α] (hb : basis b) : basis_of b ⊤ := 
...
lemma hyperplane.eq_of_le_nonspanning (hf : hyperplane f) (hx : ¬spanning x) (hdc : f ≤ x) : x=f := c := (hc.2 hd hdc).symm
...
--lemma hyperplane.not_spanning (hf : hyperplane f) : 

-- lemma indep.increment (hi : indep i) (hi_nb : ¬basis i) (his : i ≤ s) (hs : spanning s):
--   ∃ j, i < j ∧ j ≤ s ∧ indep j := 
-- begin
...
-- lemma basis_of.basis (hb : basis_of b x) (hx : spanning x) : basis b := 
-- begin
--   by_contradiction h, 
...
-- lemma exists_basis_of [order_bot α] (x : α) : ∃ i, basis_of i x :=
-- begin
--   obtain ⟨b,hb⟩ := exists_basis, 
--   obtain ⟨i,⟨⟨b,hb,hib⟩,-,hix⟩,hi'⟩ := basis_has_max_lower ⊥ x ⟨⊥,⟨b,hb,bot_le⟩,rfl.le, bot_le⟩, 
...
-- lemma basis_of_indep_spanning (hi : indep x) (hs : spanning x) : basis x := 
-- begin 
--   obtain ⟨⟨bi,hbi,hxbi⟩,⟨bs,hbs,hbsx⟩⟩ := ⟨hi,hs⟩, 
--   refine by_contra (λ h, (basis_antichain hbs hbi (λ h', h _) (hbsx.trans hxbi)).elim), 
