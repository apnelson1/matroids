...
lemma indep.le_basis_for_of_le_of_finite [base_family α] [finite α] {i x : α} (hi : indep i) 
(hix : i ≤ x) : 
  ∃ j, j basis_for x ∧ i ≤ j := 
...
(le_basis_for_of_indep_le : ∀ (i x : α), indep i → i ≤ x → ∃ j, j basis_for x ∧ i ≤ j)
...
lemma indep.le_basis_for_of_le (hi : indep i) (hix : i ≤ x) : ∃ j, j basis_for x ∧ i ≤ j := 
supermatroid.le_basis_for_of_indep_le _ _ hi hix  

lemma indep.le_basis_for_sup_right (hi : indep i) (x : α) : ∃ j, j basis_for (i ⊔ x) ∧ i ≤ j := 
hi.le_basis_for_of_le le_sup_left 

lemma indep.le_basis_for_sup_left (hi : indep i) (x : α) : ∃ j, j basis_for (x ⊔ i) ∧ i ≤ j := 
hi.le_basis_for_of_le le_sup_right 

lemma indep.lt_basis_for_of_not_basis_for_of_le (hi : indep i) (hi_n : ¬ i basis_for x) 
(hix : i ≤ x) : 
  ∃ j, j basis_for x ∧ i < j := 
(hi.le_basis_for_of_le hix).imp (λ j hj, ⟨hj.1, hj.2.lt_of_ne (λ h, hi_n (h.substr hj.1))⟩) 

lemma indep.exists_lt_inf_base_of_not_basis_for (hi : indep i) (hnb : ¬ i basis_for x) 
(hix : i ≤ x) : 
...
private lemma spanning.canopy_for_le_of_le' (hs : spanning s) (hxs : x ≤ s) : 
  ∃ t, t canopy_for x ∧ t ≤ s :=
begin
  obtain ⟨b,hb,hbs⟩ := hs, 
...
⟨ λ i s hi hs his, 
  ...
  λ i x hi hix, (spanning.canopy_for_le_of_le' hi hix).imp (λ a ⟨ha, hia⟩, ⟨ha, hia⟩)⟩
...
lemma spanning.canopy_for_le_of_le (hs : spanning s) (hxs : x ≤ s) : ∃ t, t canopy_for x ∧ t ≤ s :=
spanning.canopy_for_le_of_le' hs hxs
...
@indep.exists_lt_inf_base_of_not_basis_for αᵒᵈ _ _ _ hs hnb hix
...
@base.inf_basis_for αᵒᵈ _ _ hb x
...
lemma base.sup_canopy_for_of_inf_basis_for (hb : base b) (hbx : (x ⊓ b) basis_for x) : 
  (x ⊔ b) canopy_for x :=
begin
  by_contradiction h,
  obtain ⟨b₁,hb₁,-, hb₁x⟩ := 
...
lemma base.sup_canopy_for_iff_inf_basis_for (hb : base b):
...
begin
  ...
  rw [←hb₁.sup_canopy_for_iff_inf_basis_for] at hj,
  ...
  rw [←hb₁b₂, hb₂.sup_canopy_for_iff_inf_basis_for] at hj, 
...
lemma basis_for.basis_for_sup_mono (hix : i basis_for x) (hj : indep j) (hij : i ≤ j) :
  j basis_for (x ⊔ j) :=
begin
  obtain ⟨b, hb, hjb⟩ := hj, 
  obtain ⟨j',hj',hjj'⟩ := (hb.indep_of_le hjb).le_basis_for_of_le (le_sup_right : j ≤ x ⊔ j), 
  obtain rfl := 
