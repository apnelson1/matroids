...
See [Oxley] for a general reference of lattice_matroids, and [Diestel,et,al] for the specific axiomatization
developed here. 
...
def augmentable' {α : Type u} [semilattice_sup α] (s : set α) : Prop := 
  ∀ (a ∈ s) b, (a ∉ maximals (≤) s) → (b ∈ maximals (≤) s) → ((set.Ioc a (a ⊔ b)) ∩ s).nonempty 

open set 

def extensible {α : Type u} [preorder α] (s : set (set α)) : Prop := 
  ∀ a x,a ∈ s → a ≤ x → (maximals (≤) (λ y, a ≤ y ∧ y ≤ x ∧ y ∈ s)).nonempty

-- lemma extensible_iff_extensible' (s : set α) : extensible s ↔ extensible' s := 
-- begin
...
(ind_augment   : augmentable' ind)
(ind_extension : extensible' ind)
...
lemma dep.ge (hx : M.dep x) (hxy : x ≤ y) : M.dep y := 
...
lemma exists_basis_of (M : lattice_matroid α) (x : α) : 
  ∃ i, M.basis_of i x :=
...
lemma bases_inj {M₁ M₂ : lattice_matroid α} (hb : M₁.bases = M₂.bases)  : M₁ = M₂ := 
  by {ext, simp_rw [mem_ind_iff, indep_iff_subset_basis, ←mem_bases_iff, hb]}
...
begin
  ...
  rwa [sdiff_eq, ←compl_sup, compl_le_comm, sup_comm] at hjb'', 
end 
...
begin
  ...
  have hi'b : ((b'' ⊓ x) ⊔ (b' ⊓ xᶜ)) ≤ b'  := sup_le (hb''x.trans (inf_le_left)) inf_le_left,  
  ...
  have hii' : i ≤ (b'' ⊓ x ⊔ b' ⊓ xᶜ) ⊓ xᶜ := le_inf (le_sup_of_le_right (le_inf hib' hi.le)) hi.le, 

  have hi'i'' : (b'' ⊓ x ⊔ b' ⊓ xᶜ) ⊓ xᶜ < i'' ⊓ xᶜ, 
  by { simp only [inf_sup_right, inf_assoc, inf_compl_eq_bot, inf_bot_eq, inf_idem, bot_sup_eq], 
    --rw [inf_sup_right, inf_assoc] at hii', simp at hii', hii' is useless
    ...
    simp [inf_sup_right, @inf_assoc _ _ _ xᶜ] at h_last, 
    assumption}, 
